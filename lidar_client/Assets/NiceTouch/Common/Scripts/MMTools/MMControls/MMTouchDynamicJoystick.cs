...
{	
	...
	public class MMTouchDynamicJoystick : MMTouchJoystick, IPointerDownHandler
	{
		...
		public void MoveDynamicJoystickTo (Vector2 screenPosition) {

			// if we're in "screen space - camera" render mode
			if (ParentCanvasRenderMode == RenderMode.ScreenSpaceCamera)
			{
				_newPosition = TargetCamera.ScreenToWorldPoint((Vector3)screenPosition);
			}
			// otherwise
			else
			{
				_newPosition = (Vector3)screenPosition;
			}
